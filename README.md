...
Ein Vorteil von VSCode ist, dass die Git pulls, commits und Pushes direkt in VSCode gemacht werden können. Dazu drückt man auf der Git Symbol in der linken Seitenleiste, Staged dann die Änderungen, Commited sie und Pusht sie dann ins Online-Repo.
![](Pasted%20image%2020260225095833.png)
...
**Run**= wird ausgeführt wenn Container gebaut wird
...
## Grundbegriffe
### Service Discovery
Service Discovery liefert Clients die Verbindungsinformationen (IP und Port) zu passenden Service-Instanzen. In statischen Systemen einfach, in verteilten Systemen mit vielen dynamischen Instanzen komplex. Clients können Services per Name anfordern, während das Backend die passende Instanz auswählt.

Vernetzung verbindet Container und stellt die Kommunikation zwischen ihnen sicher. Service Discovery und Vernetzung arbeiten oft zusammen und bieten zusätzliche Funktionen wie Health Checking, Failover, Load Balancing, Verschlüsselung und Isolation von Containergruppen
### Load Balancing
Der Begriff sollte ziemlich klar sein. Es ist die Lastenverteilung zwischen verschiedenen Maschinen. Dies ist besonders bei Webservern wichtig. Im bezug auf Kubernetes ist es die Verteilung von Anfragen auf verschiede Container. 
### Cluster
Ist eine Rechnergruppe. Meist sind die Aufgaben die Erhöhung der Rechenkapazität und oder Hochverfügbarkeit. 
## Kubernetes

![](Pasted%20image%2020260225142706.png)

Kubernetes (K8s, „Kuhbernetties“) ist ein Open-Source-System zur Automatisierung von Bereitstellung, Skalierung und Verwaltung von Container-Anwendungen. Es wurde von Google entwickelt und an die Cloud Native Computing Foundation gespendet. K8s ermöglicht das automatisierte Betreiben von Containern auf verteilten Hosts und unterstützt Tools wie Docker. Führende Cloud-Plattformen wie Azure, Bluemix, OpenShift und AWS nutzen K8s.

### Merkmale
- Immutable statt veränderbar
- Deklarative statt imperative Konfiguration
- Selbstheilend: Neustart bei Absturz
- Entkoppelte APIs (LoadBalancer, Ingress)
- Skalierung durch Anpassung der Deklaration
- Anwendungsorientiertes Denken
- Abstraktion der Infrastruktur

### Objekte
- **Pod** - Ein [Pod](https://kubernetes.io/docs/concepts/workloads/pods/pod/) repräsentiert eine Gruppe von Anwendungs-Containern und Volumes, die in der gleichen Ausführungsumgebung (gleiche IP, Node) laufen.
...
### Installation
 Zuerst sollte man sich an seinem Wunschplatz einen Ordner kreieren. 
Danach:
```shell
git clone https://github.com/navilg/vagrant-kubernetes.git
```

Ich auf Windows arbeite, muss ich in der settings.yaml etwas kleines Anpassen. Die gui: muss auf "true" gesetzt werden. 
![](Pasted%20image%2020260225144305.png)

Dann kann Vagrant gestartet werden. 
```shell
vagrant up
```
